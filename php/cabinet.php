<?php
	...
	$sql="SELECT nom, adresse, acces, ville, codePostal
	...
	while($enr = mysqli_fetch_assoc($r)) {
		...
		echo '<form method=POST action="cabinet.php">';
			echo '<table border=1 cellpadding=5>';
				...
				echo gk_cb_from_ligne("<label>Donner la ville</label>", "<input type=text name=txtVille value='$ville' size=30/>", "right", "");
				echo gk_cb_from_ligne("<label>Donner le code postal</label>", "<input type=text name=txtCodePostal value='$codePostal' size=30/>", "right", "");
				echo gk_cb_from_ligne("<label>Acces</label>", "<input type=text name=txtAcces value='$acces' size=37 />", "right", "");
	...
	$sql="SELECT DISTINCT id, nom, adresse, acces
			...
			AND '$id_user' != THERA_CAB.idTherapeute ";
		$r = mysqli_query($bd, $sql) or gk_bd_erreur($bd, $sql);
		
		
	echo '<form method="GET" action="cabinet.php?id=',$id,'&amp;">';
	...
	if(isset($_POST['btnValider1'])) {
		...
		$S = "UPDATE CABINET, THERA_CAB SET 
					nom = '$nom',
					adresse = '$adresse',
					ville = '$ville',
